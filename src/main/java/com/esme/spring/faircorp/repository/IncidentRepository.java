...
public interface IncidentRepository extends JpaRepository<Incident, Integer> {
    @Query("select i.id as id, i.room.name as room, i.type as type, i.time as time, i.image as image," +
            " i.status as status from Incident i where i.room.users.id = :id order by time desc")
    List<IncidentListDTO> getListIncident(int id);

    @Query("select i.id as id, i.room.name as room, i.type as type, i.time as time, i.image as image," +
            " i.status as status from Incident i where i.room.id = :id order by time desc")
