...
import com.esme.spring.faircorp.web.dto.RoomInfoDetailDTO;
...
public interface RoomRepository extends JpaRepository<Room, Integer> {
    ...
    @Query(value = "SELECT r.id as id, r.name as name, r.area as area, r.price as price, r.number_of_tenants as numberOfTenants," +
            " r.rent_from as rentFrom, r.sex as sex FROM room r WHERE r.id = ?1", nativeQuery = true)
    RoomInfoDTO getRoomById(int id);

    @Query("SELECT r FROM Room r WHERE r.users.id = :id")
    List<Room> getRoomByUserId(int id);
